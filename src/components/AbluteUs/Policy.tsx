...
const Policy =()=>{
 ...
 Policy
      ...
      <div className="md:w-[80%] w-full border-4 rounded-lg border-red-600 bg-amber-100 text-gray-700 p-2">
        <h1 className="font-bold font-kanit text-blue-500 text-center text-2xl">Spamming</h1>
        <p className=" text-center font-josefin">
        ‘Spamming’ is an unsolicited bulk and/or commercial email sent through the Internet without the recipient’s consent. It is bad not only because it has a poor effect on consumer perceptions of Wincloud, but also because it can overburden the company’s network and interfere with service to its clients’ attitudes towards our firm.</p>
      </div>

      <div className="md:w-[80%] w-full border-4 rounded-lg border-red-600 bg-amber-100 p-2">
        <h1 className="font-bold font-kanit text-blue-500 text-center text-2xl">Intellectual property violations
        </h1>
        <p className="font-josefin text-center">
        Taking part in any action that violates or abridges another party’s intellectual property rights, including patents, copyrights, proprietary information, logos, registered trademark, and brand names owned by businesses, people, or other organizations.</p>
      </div>

      <div className="md:w-[80%] w-full border-4 rounded-lg border-red-600 bg-amber-100 p-2">
        <h1 className="font-bold font-kanit text-blue-500 text-center text-2xl">Facilitating a Violation of this AUP
        </h1>
        <p className="font-josefin text-center">
        This AUP prohibits the use of pings, floods, mail bombs, and denial-of-service attacks, as well as in some way providing software, programs, services, or resources that violate it.</p>
      </div>
      <div className="md:w-[80%] w-full border-4 rounded-lg border-red-600 bg-amber-100 p-2">
        <h1 className="font-bold font-kanit text-blue-500 text-center text-2xl">Other illegal activities
        </h1>
        <p className="font-josefin text-center">
        Taking part in actions that have been deemed to be unlawful, such as promoting, disseminating, or otherwise creating Ponzi and pyramid schemes, as well as fraudulently charging credit cards and software piracy.
        </p>
      </div>
      <div className="md:w-[80%] w-full border-4 rounded-lg border-red-600 bg-amber-100 p-2">
        <h1 className="font-bold font-kanit text-blue-500 text-center text-2xl">Export control violations
        </h1>
        <p className="font-josefin text-center">
        Exporting encryption software outside of the United States, via the Internet or otherwise.</p>
      </div>
      <div className="md:w-[80%] w-full border-4 rounded-lg border-red-600 bg-amber-100 p-2">
        <h1 className="font-bold font-kanit text-blue-500 text-center text-2xl">Obscene speech or Materials
        </h1>
        <p className="font-josefin text-center">
        Advertisement, transmission, storage, display, or other dissemination of child pornography or obscene words or expressions on Winscloud’s network is not permitted.</p>
      </div>
    </div>
    
    <h2 className="p-3 text-white rounded-xl text-center md:text-start bg-red-600 border mt-5 font-kanit font-semibold text-2xl">Renewal Term and Termination Conditions:
    </h2>
    
    <div className=" border-4 bg-amber-100   rounded-lg border-red-600 font-josefin mt-4 p-2  text-gray-700">
    
    <p className="my-6">
    
    Upon activation and availability of Customer “user profiles” on Winscloud Service, the contract shall be effective on the date of activation and continue to be effective for one (1) month from the effective date. To terminate service, customers must send written notice at least 15 days before the termination date to accounting[at]winscloudmatrix.com. If either party terminates this User Acceptance Policy agreement earlier than the expiration of the initial Term or any subsequent Term, it will automatically renew for one (1) month. Customer data will be removed from active file systems of Winscloud terminal servers upon termination of services by Customer or termination of services by Winscloud. When Winscloud terminates an account, customers may request that Winscloud Matrix retain and back up their data for 7 days only. All searches from terminated customer accounts will be subject to time and materials charges. Only the previously authorized administrative contact for the Customer account will be able to make such a request for data search or retrieval.
</p>
      </div>
    
    
      <h2 className="p-3 text-white rounded-xl text-center md:text-start bg-red-600 border mt-5 font-kanit font-semibold text-2xl">No warranties:
      </h2>
    

      <div className=" border-4 font-josefin rounded-lg border-red-600 mt-4 p-2 bg-amber-100 text-gray-700">
        <p >
        If Customer user profiles are activated and made available for use on Winscloud Service, the Effective Date shall be the date they are activated. This Agreement will be in full force and effect for an initial Term of one (1) month following the Effective Date. It is possible for customers to terminate their service at any time by submitting a written notice 15 days in advance to accounting[at]winscloudmatrix.com. This User Acceptance Policy agreement for Custom/Private Cloud environments will keep renewing for one (1) month unless either party informs the other party in writing before the end of the original Term or any subsequent Term. Winscloud will delete all Customer data from active file systems on its terminal servers upon the termination of Service by Customer or termination of Service by Winscloud. When Winscloud’s terminal servers will retain a copy of the customer’s data for a period of 7 days following termination of the account. Only the previously approved administrative contact for the Customer account is allowed to conduct searches from terminated Service accounts of customers.</p>
    
    
      </div>
    
      <h2 className="p-3 text-white rounded-xl text-center md:text-start bg-red-600 border mt-5 font-kanit font-semibold text-2xl">Limitation of Liability
      </h2>
      ...
      <h2 className="p-3 text-white rounded-xl text-center md:text-start bg-red-600 border mt-5 font-kanit font-semibold text-2xl">You agree with user acceptance policy:
      </h2>
